...
class AnalyticsDaemon(BaseDaemon):
    ...
    def updateFilesSummaries(self):
        ...
        try:
            ...
            return
        for fileDoc in all_files:
            if self.monitoring_db.documentExists(fileDoc['value']['id']):
                doc = self.monitoring_db.document(fileDoc['value']['id'])
            else:
                doc = {}
            doc['type'] = 'aso_file'
            doc['_id'] = fileDoc['value']['id']
            doc['workflow'] = fileDoc['value']['workflow']
            doc['lfn'] = fileDoc['value']['lfn']
            doc['location'] = fileDoc['value']['location']
            doc['checksum'] = fileDoc['value']['checksum']
            doc['jobid'] = fileDoc['value']['jobid']
            doc['retry_count'] = fileDoc['value']['retry_count']
            doc['size'] = fileDoc['value']['size']
            doc['state'] = fileDoc['value']['state']
            # TODO: For backward compatibility. Remove it later
            if 'publication_state' in fileDoc['value']:
                doc['publication_state'] = fileDoc['value']['publication_state']
            # doc['publication_state'] = fileDoc['value']['publication_state']
            doc['last_update'] = time.time()
            if 'publish' in fileDoc['value']:
                doc['publish'] = fileDoc['value']['publish']
            else:
                doc['publish'] = 0
            if fileDoc['value']['failure_reason']:
                doc['failure_reason'] = fileDoc['value']['failure_reason']
            if 'type' in fileDoc['value']:
                doc['file_type'] = fileDoc['value']['type']
            else:
        ...
        for job in all_jobs:
            ...
            if ended_files:
                ...
                if number_ended_files == job_doc['files']:
                    ...
                    for fileDoc in done_files:
                        status[fileDoc['value'].replace('store', 'store/temp', 1)] = 'done'
                    if 'log_file' in job_doc:
                    ...
                    if len(done_files) != number_ended_files:
                        ...
                        transferError = ""
                        for fileDoc in failed_files:
                            if fileDoc['value'].find('temp') > 1:
                                status[fileDoc['value']] = 'failed'
                                lfn = fileDoc['value']
                                ...
                                lfn = fileDoc['value'].replace('store', 'store/temp', 1)
                                status[lfn] = 'failed'
                                docId = getHashLfn(lfn)
                                # Load document to get the failure reason from output file
                                try:
                    ...
                    if (len(done_files) + len(failed_files)) != number_ended_files:
                        ...
                        for fileDoc in cancelled_files:
                            if fileDoc['value'].find('temp') > 1:
                                status[fileDoc['value']] = 'cancelled'
                            else:
                                status[fileDoc['value'].replace('store', 'store/temp', 1)] = 'cancelled'
                        message['transferStatus'] = status
    ...
    def cleanOldDocs(self):
        ...
        for old_fileDoc in files_to_remove:
            try:
                document = self.monitoring_db.document(old_fileDoc['value'])
            except:
                self.logger.debug('%s does not exist and cannot be remove', old_fileDoc['value'])
            try:
                self.monitoring_db.queueDelete(document)
                self.logger.debug('%s removed' % old_fileDoc['value'])
            except:
                self.logger.debug('%s cannot be remove from couch', old_fileDoc['value'])
        try:
            self.monitoring_db.commit()
