...
class TransferWorker(object):
    ...
    def __call__(self):
        ...
        dummystdout, rc = execute_command(command, self.logger, self.commandTimeout)
    ...
    def apply_tfc_to_lfn(self, fileDoc):
        """
        Take a CMS_NAME:lfn string and make a pfn.
        Update pfn_to_lfn_mapping dictionary.
        """
        try:
            site, lfn = tuple(fileDoc.split(':'))
        except:
            self.logger.error('it does not seem to be an lfn %s', fileDoc.split(':'))
            return None
    ...
    def command(self, jobs, jobs_lfn, jobs_pfn, jobs_report):
        ...
        for link, copyjob in jobs.items():
            ...
            if not self.validate_copyjob(copyjob):
                continue
            ...
            for lfn in fts_job['LFNs']:
                ...
                job_id = '%d_https://glidein.cern.ch/%d/%s_%s' % (int(jobs_report[link][index][0]), int(jobs_report[link][index][0]),
                                                                  lfn_report['Workflow'].replace("_", ":"), lfn_report['JobVersion'])
    ...
    def validate_copyjob(self, copyjob):
        ...
        """
        for task in copyjob:
            if task.split()[0] == 'None' or task.split()[1] == 'None':
                return False
        return True
