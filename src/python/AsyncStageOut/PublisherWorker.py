...
# Not nice to disable W0703, but all exceptions should be change to proper ones. TODO: fix it.
'''
...
class PublisherWorker(object):
    ...
    def format_file_3(self, fileDoc):
        nf = {'logical_file_name': fileDoc['lfn'],
              'file_type': 'EDM',
              'check_sum': unicode(fileDoc['cksum']),
              'event_count': fileDoc['inevents'],
              'file_size': fileDoc['filesize'],
              'adler32': fileDoc['adler32'],
              'file_parent_list': [{'file_parent_lfn': i} for i in set(fileDoc['parents'])]}
        file_lumi_list = []
        for run, lumis in fileDoc['runlumi'].items():
            for lumi in lumis:
                file_lumi_list.append({'lumi_section_num': int(lumi), 'run_num': int(run)})
        nf['file_lumi_list'] = file_lumi_list
        if fileDoc.get("md5") != "asda" and fileDoc.get("md5") != "NOTSET":  # asda is the silly value that MD5 defaults to
            nf['md5'] = fileDoc['md5']
        return nf
    ...
    def createBulkBlock(self, output_config, processing_era_config, primds_config,
        ...
        for fileDoc in files:
            ...
            file_conf['lfn'] = fileDoc['logical_file_name']
            for parent_lfn in fileDoc.get('file_parent_list', []):
                file_parent_list.append({'logical_file_name': fileDoc['logical_file_name'],
                                         'parent_logical_file_name': parent_lfn['file_parent_lfn']})
            del fileDoc['file_parent_list']
        blockDump = {
        ...
        blockDump['block']['block_size'] = sum([int(fileDoc[u'file_size']) for fileDoc in files])
        return blockDump
    ...
    def publishInDBS3(self, workflow, sourceURL, inputDataset, toPublish, pnn):
        ...
        for dataset, files in toPublish.iteritems():
            ...
            dummyempty, primName, procName, tier = dataset.split('/')
            ...
            workToDo = False
            for fileDoc in files:
                if fileDoc['lfn'] not in existingFilesValid:
                    workToDo = True
                    break
            ...
            for fileDoc in files:
                ...
                if fileDoc['lfn'] not in existingFilesValid:
                    ...
                    for parentFile in list(fileDoc['parents']):
                        ...
                        if parentFile in parentsToSkip:
                            ...
                            self.logger.info(wfnamemsg + msg)
                            if parentFile in fileDoc['parents']:
                                fileDoc['parents'].remove(parentFile)
                    # Add this file to the list of files to be published.
                    dbsFiles.append(self.format_file_3(fileDoc))
                published[dataset].append(fileDoc['lfn'])
