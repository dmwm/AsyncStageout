function(doc) {
	if(doc.end_time){
		...
		m_utc = startDate.getUTCMinutes();
 		...
 		if (doc.state=='new'){
                 if ( mm == "12"){
                        emit([parseInt(yy), parseInt(mm), parseInt(dd), parseInt(h), parseInt(m), parseInt(s)], {"state": 'resubmitted'});
                 }
                 else {
                        emit([yy_utc, mm_utc, dd_utc, h_utc, m_utc, s_utc], {"state": 'resubmitted'});
